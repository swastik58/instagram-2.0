...
function Post({ id, username, userImg, img, caption }) {
    ...
    return (
        <div className="bg-white my-7 border rounded-sm">
            ...
            {/* likes and comments */} {/* after connection to firebase */}
            {comments.length > 0 && (
                <div className="ml-10 h-20 overflow-y-scroll scrollbar-thumb-black scrollbar-thin">
                    {comments.map(comment => (
                        <div key={comment.id} className="flex items-center space-x-2 mb-3">
                            <img className="h-7 rounded-full" src={comment.data().userImage} alt="profile image" />
                            <p className="text-sm flex-1"><span className="font-bold"></span>{" "}{comment.data().comment}</p>
            ...
            {session && (
                <form className="flex items-center p-4">  
                    ...
                    <input type="text" value={comment} onChange={e=> setComment(e.target.value)} placeholder="Add a comment..." className="border-none flex-1 focus-ring:ring-0 outline-none" />
